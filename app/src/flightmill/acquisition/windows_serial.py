...
The application serial worker selects this backend on Windows, reusing the
accepted Checkpoint 02 capture correction. No global patch or installed-package
edit is performed. Original source: hardware/validation/windows_reset_serial.py.
The open/handle setup is adapted from pySerial 3.5 serialwin32.py; see
pyserial-LICENSE.txt. Reads, writes, settings, and normal close
...
class WindowsResetSerial(Serial):
    ...
    def open(self):
        ...
        if handle == win32.INVALID_HANDLE_VALUE:
            ...
            message = f"Cannot open reset capture port {port}: {error}"
            if error.winerror == 2:  # ERROR_FILE_NOT_FOUND; no handle exists yet.
                raise SerialPortNotReadyError(message) from error
            raise SerialException(message) from error
